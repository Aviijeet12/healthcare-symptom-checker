...
- [ ] Copy `healthcare-backend/.env.example` to `healthcare-backend/.env` and set `OPENAI_API_KEY` (+ optional overrides)
...
- [ ] Confirm Render env vars `OPENAI_API_KEY` (and `OPENAI_MODEL` if used) are present
